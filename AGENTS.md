...
A Zotero 7+ bootstrapped plugin that registers eight HTTP endpoints on Zotero's own `127.0.0.1:23119` server, plus a Python client (`zoterojs.py`) for driving them. There is no build step for the plugin itself — Zotero loads `addon/bootstrap.js` directly.
...
- **All writes go through `doApply`**, which supplies the backup and the `collectionsLost` / `parentCollectionsGained` diffing.
...
5. Documentation: the endpoint table in `README.md` **and** `README.zh-CN.md`, plus `docs/endpoints.md`.
